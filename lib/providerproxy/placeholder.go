...
// Media bodies are never committed: a cover is hundreds of kilobytes and an
// episode is tens of megabytes, and neither is what these tests are about.
// They are elided at record time and replaced on replay by the smallest valid
// file of the same kind, so Audiobookshelf still gets something it can decode
// and save - which is the behaviour under test - without the repository
// carrying a provider's artwork.
//
